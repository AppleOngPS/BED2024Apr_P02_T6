  ...
  document.getElementById("saveButton").addEventListener("click", async () => {
    try {
      const updatedUserData = {
        ...
        contactNumber: document.getElementById("exampleInputMobileNumber")
          .value,
        age: parseInt(document.getElementById("exampleInputAge").value, 10),
        height: parseInt(
          document.getElementById("exampleInputHeight").value,
          10
        ),
        weight: parseInt(
          document.getElementById("exampleInputWeight").value,
          10
        ),
        weightGoal: parseInt(
          document.getElementById("exampleInputWeightGoal").value,
          10
        ),
        TargetCalarieIntake: parseInt(
          document.getElementById("exampleInputTargetCalorieGoal").value,
          10
        ),
      };
    ...
    .addEventListener("click", async () => {
      if (
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to delete user account. Status: ${response.status}, ${response.statusText}`
            );
          }
...
function displayUserData(userData) {
  ...
  document.getElementById("exampleInputMobileNumber").value =
